...
public class Calculator extends JFrame {
	...
	public Calculator() {
		//계산기의 화면과 버튼을 붙여주기 기본 레이아웃 사용
		setLayout(null);
		
		//빈공간의 JTextField 생성
		inputSpace = new JTextField();
		//편집 가능 여부는 불가능으로 설정(버튼으로만 입력 받기)
		...
		//위치와 크기 설정하기 (위치 = x:8 , y:10, 크기 = 270x70)
	...
	class PadActionListener implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			...
			//지금 누른 버튼이 연산자일 때의 조건
				...
				//이전에 누른 버튼이 연산자가 아니고 위의 계산식이 비어있지 않을 때의 조건문 생성
	...
	// 계산기능을 구현하기 위해 ArryList 에 숫자와 연산 기호를 하나씩 구분에 담아주기
	private void fullTextParsing(String inputText) {
		...
		for (int i = 0; i < inputText.length(); i++) {
			...
			//연산 기호를 거치게 될 식앞은 숫자이므로 숫자를 먼저 ArrayList 에 추가하기
			if (ch == '-' || ch == '+' || ch == '×' || ch == '÷') {
				//숫자를 ArrayList에 추가
				equation.add(num);
				//num 초기화
		...
		//반복문 끝나고 남아있는 숫자값 추가
		equation.add(num);
		//연산자가 있을 때 num을 ArrayList에 추가하는데, 처음에 -가 있으면 ""가 추가되어 에러가 발생함 즉 ""을 제거
		equation.remove("");
	}
	...
	public double calculate(String inputText) {
		...
		for (String s : equation) {
			...
			//Math.round(n*(10 * 표시할 자릿수)) / 10 * 표시할 자릿수로 반올림할 자릿수 조절
